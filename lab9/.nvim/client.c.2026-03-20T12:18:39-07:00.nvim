...
Answer: The address of the server it is trying to connect to is 127.0.0.1 on port 8000


2. Is it UDP or TCP? How do you know?
Answer: It is TCP because it uses SOCK_STREAM, meaning it is a TCP socket

3. The client is going to send some data to the server. Where does it get this data from? How can
you tell in the code?
Answer: It gets the data from standard input as it as seen by the line ((num_read =
read(STDIN_FILENO, buf, BUF_SIZE)) > 1

4. How does the client program end? How can you tell that in the code?
Answer: The client ends with calls to close and exit
*/
