...
A: The client is sending data to the server using write(sfd, buf, BUF_SIZE), while using a TCP socket over IPv4

2. What data is the client sending to the server?
A: The data the client sends to the server is messages such as Hello, Apple, Car, Green, and Dog.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
A: The argument is a struct that contains a flag that marks whether the thread should run, a pointer to theshared linked list where received messages are stored, and a pointer to the mutex lock protecting the list

2. How are received messages stored?
A: Received messages are stored in a shared linked list

3. What does `main()` do with the received messages?
A: main() initializes the mutex and shared list, starts the acceptor thread, waits until all expected messages are received, stops the acceptor thread, then prints and frees all stored messages

4. How are threads used in this sample code?
A: The main thread controls the whole program, while an acceptor thread accepts incoming client connections, and a client thread is created for each connected client to read messages and add them to the shared list
*/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
